...
import {set3, sub3, normalize} from "../_snowpack/pkg/@thi.ng/vectors.js";
import {
  Euler,
  Matrix4,
  Quaternion,
...
const quatTest = () => {
  ...
  const helper = new ArrowHelper(new Vector3(), void 0, 2);
...
const lookAtTest = () => {
  ...
  const helper2 = new ArrowHelper(void 0, void 0, 2, "blue");
  group.add(helper2);
  return group;
};
const instanceTest = () => {
  ...
  const group = new Group();
  const count = 20;
  const ins = createInstancedMesh(geom, new MeshBasicMaterial({
    color: "yellow"
  }), count);
  const pnt = createInstancedMesh(pgeom, new MeshBasicMaterial({
    color: "white"
  }), count);
  const iterator = instancedMeshIterator(ins);
  const quat = new Quaternion();
  const helper = new ArrowHelper(new Vector3(1, 0, 0), void 0, 3, "hotpink");
  group.add(helper);
  let prev = [0, 0, 0];
  iterator((idx, pos, scal, rot) => {
    ...
    quat.setFromUnitVectors(new Vector3().fromArray(dir).normalize(), new Vector3());
