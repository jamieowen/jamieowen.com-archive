...
import { subdivide, BoundsInfo } from "../src/__old2/layouts/subdivide";
import { subdivideOnCondition } from "../src/__old2/layouts/subdivideOnCondition";
import { grid, gridOffset } from "../src/__old2/layouts/grid";
import { basketweave } from "../src/__old2/layouts/basketweave";
import { matrixRepeater } from "../src/__old2/layouts/matrixRepeater";
...
function splitByRatio(node: BoundsNode, ratio: number) {
  ...
  if (w1 === 0 || w2 === 0) {
    ...
    return w > h
      ? { w: 2, h: 1, xw: [w1, w2], yw: [1] }
      : { w: 1, h: 2, yw: [w1, w2], xw: [1] };
  }
}
...
const subdivideNode = (node) => {
  ...
  [
    ...subdivideOnCondition(
      node,
      createNode,
      (target: BoundsNode, depth: number) => {
        ...
        if (nodeArea - nodeArea * 0.2 > minArea) {
          ...
          if (nodeArea * 0.8 < minArea) {
            // 20% padding? no subdiv..
            return false;
            // return w > h ? { w:5,h:1 } : { w:1,h:5 };
          } else if (nodeArea * 0.6 < minArea) {
            console.log("DIV 2");
...
const render = () => {
  ...
  let iterator = null;
  let gridOpts = {
    w: options.tilingWidth,
    h: options.tilingHeight,
