...
import { tickStream, TickEvent } from "../streams/clock-stream";
import {
  resizeObserverStream,
  ResizeEvent,
} from "../streams/resize-observer-stream";
...
export function renderStream(domElement: HTMLElement, opts: RenderOpts = {}) {
  ...
  }).transform<RenderEvent>(addToDom(), resizeRenderer(), resizeCamera());
  ...
  let gevent = null;
  gestures.transform(map((e) => (gevent = e)));
  ...
  return stream.transform(
    map((e: RenderEvent) => {
      if (gevent && gevent.pos) {
        ...
        const intersects = intersect.raycaster.intersectObjects(
          e.scene.children,
          true
        );
