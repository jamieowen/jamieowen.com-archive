...
import { BranchNode,GroupNode,BoundsNode,PointNode,RectNode, SubDivideNode} from '../nodes';
...
export const group = (rfn:childRfn=noop):DeferredNodeDecl => new DeferredNodeDecl(rfn,GroupNode);
...
export type BoundsInfoFunc = (i:number,
  x:number,y:number,
  w:number,h:number,
) => DeferredNodeDecl;

export function subdivide( opts:SubDivideOpts,createNode:BoundsInfoFunc ):DeferredNodeDecl{
